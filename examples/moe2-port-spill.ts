/**
 * GODKJENNINGSPORTEN FOR KORTFASITEN – hvor i runden er orakelet verdt å følge?
 *
...
function énRunde(v: Vindu, sete: number, frø: number): { poeng: number; orakelvalg: number } {
  ...
  while (s.fase !== "FERDIG" && s.fase !== "RUNDE_SLUTT" && vakt++ < 20_000) {
    ...
    if (iTur === sete && s.fase === "SPILL" && s.stikkSpilt >= v.fra && s.stikkSpilt <= v.til) {
      // Samme solverinnstilling som spillfasiten i treningen bruker. Poenget
      // er aa teste den fasiten vi faktisk kan traene paa, ikke en dypere
      // loesning vi ikke har raad til per beslutning.
...
console.log(`\n=== Aa foelge solveren i ulike stikk, ${givere} givere x 4 seter ===`);
...
writeFileSync("analyse/moe2-port-spill.json", JSON.stringify({ givere, rader, dom }, null, 2));
console.log(`\nSkrev analyse/moe2-port-spill.json`);
