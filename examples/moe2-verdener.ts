/**
 * HULLET I STIKK 0-2: er det for faa verdener?
 *
...
/**
 * VERDENSSVEIP I DET TIDLIGE VINDUET.
 *
 * Baade DD (-0,62) og SD (-0,88) taper poeng naar de foelges i stikk 0-2. Min
 * forklaring var at fire verdener ikke dekker mulighetsrommet naar mest er
 * skjult. Det er en PAASTAND. Her testes den: hjelper flere verdener tidlig?
 *
 * Kontrollen er det sene vinduet med samme verdenstall - er forbedringen der
 * like stor, handler det om verdener generelt og ikke om tidlige stikk.
 */
const vinduer: Vindu[] = [
  { navn: "aldri (nevro)", fra: 99, til: 99, verdener: 4 },
  { navn: "0-2, 4 verd.", fra: 0, til: 2, verdener: 4 },
  { navn: "0-2, 12 verd.", fra: 0, til: 2, verdener: 12 },
  { navn: "0-2, 32 verd.", fra: 0, til: 2, verdener: 32 },
  { navn: "6+, 4 verd.", fra: 6, til: 99, verdener: 4 },
  { navn: "6+, 32 verd.", fra: 6, til: 99, verdener: 32 },
];
...
function énRunde(v: Vindu, sete: number, frø: number): { poeng: number; orakelvalg: number } {
  ...
  while (s.fase !== "FERDIG" && s.fase !== "RUNDE_SLUTT" && vakt++ < 20_000) {
    ...
    if (iTur === sete && s.fase === "SPILL" && s.stikkSpilt >= v.fra && s.stikkSpilt <= v.til) {
      ...
      const kort = besteKortSD(s, sete, nevro, { verdener: v.verdener, rng });
      if (kort !== null) {
        orakelvalg++;
...
console.log(`\n=== Hjelper flere verdener tidlig i runden?, ${givere} givere x 4 seter ===`);
...
writeFileSync("analyse/moe2-verdener.json", JSON.stringify({ givere, rader, dom }, null, 2));
console.log(`\nSkrev analyse/moe2-verdener.json`);
