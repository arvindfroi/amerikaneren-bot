 ...
 *
 *   node examples/pulje-identitet.ts --ut analyse/pulje-id-etter.txt
 *   # og i en EGEN arbeidskopi på grunnlinjecommiten:
 *   node examples/pulje-identitet.ts --ut analyse/pulje-id-foer.txt
 *   diff analyse/pulje-id-foer.txt analyse/pulje-id-etter.txt
 *
 * ============ HVORFOR DENNE OG IKKE EN PRØVE ==============================
 *
 * En prøve inne i grenen kan bare vise at knotten AV oppfører seg som knotten AV. Den
 * kan ikke vise at den oppfører seg som KODEN FØR KNOTTEN FANTES — og det er nøyaktig
 * det «av er av» betyr. Denne fila skriver derfor et fingeravtrykk som regnes ut på
 * BEGGE commitene og sammenliknes med `diff`. Består den, hviler bit-identiteten på at
 * grenen ikke kjøres, ikke på at den regner riktig.
 *
 * Formen er strata-agentens (`D:\amb-strata`, `examples/strata-identitet.ts`), som i sin
 * tur er bandit-agentens. Den er god, og den er ikke verdt å finne opp på nytt.
 *
 * TO TING SOM IKKE ER TILFELDIGE:
 *
 *   1. **48 VERDENER, ikke 16.** Utvelgelsen er en funksjon AV antall verdener — cellene
 *      er `1/antall` brede, og hvilke kandidater som får flere slott avhenger av det. Et
 *      fingeravtrykk på et annet K ville hatt et hull nøyaktig der speken bor.
 *   2. **Ingen `pulje`-nøkkel i det hele tatt** — ikke `pulje: undefined`, men fraværende.
 *      Det er formen `Sikkerorakel` faktisk bruker (`...(pulje === null ? {} : …)`).
 *
 * Fingeravtrykket dekker alt `Sikkerorakel` leser ut av `vurderPar`: valgt kort, antall
 * verdener, σ, margin OG hver enkelt verdi per kandidat per verden. Bare kortet ville
 * sluppet gjennom en endring som flyttet verdiene uten å snu argmaks — og det er
 * nettopp det en trekkeendring GJØR, så per-verden-verdiene er ikke pynt her, de er
 * hele beviset. 120 stillinger × ~4 kandidater × 48 verdener er i størrelsesorden
 * 23 000 flyttall som alle skal være bit-like.
 */
...
while (s.fase !== "FERDIG" && vakt++ < 40_000 && n < STILLINGER) {
  ...
  if (s.fase === "SPILL" && s.iTur !== null && lovligeKort(s, s.iTur).length >= 2) {
    ...
    const par = vurderPar(s, sete, indre[sete]! as unknown as Utspiller, {
      ...
      // INGEN `pulje`. Nøkkelen skal ikke finnes i objektet i det hele tatt.
    });
    if (par !== null) {
      n++;
