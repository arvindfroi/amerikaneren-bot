...
TERSKLER="25000 50000 100000 200000"
...
for T in $TERSKLER; do
  ...
  for S in 0 1 2 3 4 5 6 7; do
    node examples/gate2.ts \
      ...
      --ut "analyse/g2-${NAVN}-${S}.jsonl" >> "$TRENLOGG" 2>&1 &
  done
  wait
  node examples/gate2.ts --rapport "analyse/g2-${NAVN}-*.jsonl" >> "$TRENLOGG" 2>&1
  ekko "RESULTAT $NAVN:"
  sed -n '1,40p' "analyse/g2-${NAVN}.txt" 2>/dev/null | tee -a "$LOGG"
done
