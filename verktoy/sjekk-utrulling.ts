...
/** Hvert filnavn appen nevner. Utledet, ikke skrevet ned. */
...
function lokalt(f: string): string | null {
  for (const p of [`web/dist/${f}`, `e1-modell/${f}`, f]) if (existsSync(p)) return p;
  return null;
}
...
for (const f of [...navn].sort()) {
  ...
  try {
    ...
    ute = buf.length;
    const start = buf.subarray(0, 32).toString("utf8").trimStart().toLowerCase();
    ...
    if (merknad === "" && sti !== null) {
      ...
      merknad = merknad === "" ? "ingen lokal kopi å sammenlikne med" : merknad;
    }
  } catch (feil) {
    status = "FEIL";
...
linjer.push("#");
linjer.push("# En 200-status betyr INGENTING her: bud-vant.json svarte 200 med");
