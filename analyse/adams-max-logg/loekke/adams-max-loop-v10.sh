#!/usr/bin/env bash
# ADAMS MAX-LØKKA v4 (12. sep, fra iterasjon 3) – som v3, men hukommelsen er ALLTID nullet i menneskeradene (--minne-dropout 1.0): i iterasjon 2 ga 0,5 negativ minnegevinst mot mennesker (K6-menneske stigning z −5,3, fremmed bok bedre enn egen). Fra v3:: befolkning + SANSER 2 + MENNESKERADER i troen.
#
#   bash /d/amb-imit/adams-max-loop-v3.sh <iterasjon>      (2, 3, …)
#
# Endringer fra v2:
...
logg "START (v5, kortnett 493) policy=$POL befolkning=3 bord, sanser2, menneskerader"
...
node examples/mlb-krav.ts --spek "$HEL" --kjerner 20 --to-band --ut "D:/amb-grp/loop/iter$K/krav" > "$D/krav.log" 2>&1
...
if [ -z "$k1ny" ]; then
  ...
  # MARGINEN ER FAST 0,15 pp, ikke 2 SE av summen. Begge målingene spiller de SAMME 2641 menneskerundene, så
  # forskjellen er langt mer presis enn ±0,19 hver: en uavhengighetsantakelse ga 0,52 pp slingring og ville
  # sluppet fallet +1,12 → +0,70 (iterasjon 4) rett gjennom. 0,15 fanger et slikt fall og tåler småsvingninger.
